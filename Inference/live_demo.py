...
def preprocess_frame_for_inference(frame):
    ...
    return final_img
# ==========================================
# 2. 模型與環境初始化 (啟動 RTX 3060 FP16 加速)
# ==========================================
class SquarePad:
...
# ==========================================
# 3. 設定螢幕擷取區域 (針對 scrcpy 視窗)
# ==========================================
monitor_region = {"top": 200, "left": 100, "width": 600, "height": 800}
print(f"🎥 開始擷取螢幕區域: {monitor_region}")
...
with mss.mss() as sct:
    while True:
        ...
        # 3. YOLO 偵測
        results = yolo_model.predict(source=inference_frame, conf=0.05, iou=0.45, half=True, verbose=False)
        for result in results:
            ...
            for box in boxes:
                ...
                with torch.no_grad():
                    ...
                    probabiities = torch.nn.functional.softmax(outputs, dim=1)[0]
                    confidence, preds = torch.max(probabilities, 0)
                    label = class_names[preds.item()]
                    conf_score = confidence.item()

            
                # 終極守門員：濾除背景與低於 40% 的假警報
        ...
        cv2.putText(origina_frame, f"FPS: {fps:.1f}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        # 5. 原生視窗顯示結果
        cv2.imshow("Live Thermal Edge Detection (RTX 3060)", original_frame)
